...
-- 用 DROP + CREATE 重建 schema。
--
-- 套用方式： ./run.sh db-reset
-- ⚠️ 需與 docker/init/01-schema-seed.sql 保持一致。
-- ⚠️ 一旦變更本 golden 資料，下次啟動請先 ./run.sh db-reset（factory reset）。
--
-- 設計要點：
--   - 時間全在 2026 年且不晚於 6/30（本輪要求）。id 與時間同序（item5）：transactions / audit_log
--     以單一 INSERT 依 created_at 升序，AUTO_INCREMENT 主鍵順序即時間順序。
--   - 每帳戶皆有 phone / address（item3 必填）；電話為身分識別（item5）：
--     同一人（電話）最多一個台幣(A)+一個外幣(B)。golden 中 alice/bob/eve 各持 A+B（同電話，合法）。
--   - 6 個 user、9 個帳戶（含 1 個 CLOSED 範例 A006）、多種交易與 audit action。
--
-- 可斷言事實：
--   使用者 6（皆 ROLE_USER，密碼 user1234）：alice/bob/carol/david/eve/frank；admin 走 in-memory。
--   帳戶 9：A001(alice)850 / A002(bob)950.50 / A003(carol)0 FROZEN / A004(david)12345.67 /
--          A005(eve)2200 / A006(frank)0 CLOSED / B001(alice)4700 / B002(bob)2300 / B003(eve)1000
--   電話：alice 0911-111-111、bob 0922-222-222、carol 0933-333-333、david 0944-444-444、
--        eve 0955-555-555、frank 0966-666-666
--   交易 42、audit_log 40（含 E2001/E2004/E1002/E5003/E6003 失敗；含豐富的 profile 變更歷史）
-- ============================================================
...
-- ---- 使用者（6 筆；密碼皆 user1234 的 BCrypt $2a$ 雜湊）----
...
-- ---- 帳戶（9 筆；每筆含 phone/address；A006 CLOSED）----
...
-- ---- 交易（單一 INSERT，依 created_at 升序 → tx_id 與時間同序）----
...
-- ---- 稽核紀錄（單一 INSERT，依 created_at 升序 → id 與時間同序；多種 action + 失敗錯誤碼）----
